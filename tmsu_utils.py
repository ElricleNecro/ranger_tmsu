...
class Tmsu:
    ...
    def tag(self, fileName, tagName, value=None):
        try:
            self._cmd(
                'tag "{}" {}{}'.format(fileName, tagName, "=" + value if value else "")
            )
            return True
        except sp.CalledProcessError as e:
    ...
    def untag(self, fileName, tagName, value=None):
        try:
            self._cmd(
                'untag "{}" {}{}'.format(
                    fileName, tagName, "=" + value if value else ""
                )
            )
            return True
        except sp.CalledProcessError as e:
    ...
    def _cmd(self, cmd):
        return sp.check_output("tmsu " + cmd, shell=True, stderr=sp.DEVNULL).decode(
            "utf-8"
        )
